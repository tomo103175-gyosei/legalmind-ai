...
export async function POST(req: Request) {
  try {
    ...
    if (userPlan === "FREE") {
      prompt += `
      7. 各選択肢（1〜5）について、正誤のポイントを条文の趣旨に沿って2〜3行で簡潔に説明してください。
      8. 解説の最後に必ず「※詳細はプレミアムプランで確認できます」という一文を単独の行で追記してください。
      9. 出力は必ず以下のJSON形式で行ってください。
      ...
      prompt += `
      7. 選択肢がなぜ正解・不正解なのかを、e-Govの条文や関連判例に基づいて「徹底的かつ詳細に」解説してください。
      8. 【最重要事項】どれほど詳細に解説する場合でも、条文・判例の「長文の直接引用（コピペ）」は絶対に禁止です。必ずあなた自身の言葉で要約・再構成してください。
      9. 行政書士試験の過去問解説として相応しい、格調高く正確な文章で記述してください。
