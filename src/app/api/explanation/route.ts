...
export async function POST(req: Request) {
  try {
    ...
    if (isFree) {
      prompt += `
      3. 【重要】現在のユーザーは「無料プラン」です。詳細な解説テキストは一切出力せず、正解の選択肢番号と、関連する「条文番号（例: 民法第90条）」およびそのe-Gov URLのみを簡潔に提示してください。
      `;
    } else {
      prompt += `
      3. 【重要】ユーザーは「プレミアムプラン」です。選択肢がなぜ正解・不正解なのかを、e-Govの条文や関連判例に基づいて徹底的かつ詳細に解説してください。
      `;
    }
